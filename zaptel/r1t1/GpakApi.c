...
static int CheckDspReset(
    struct r1t1 *rh, /* Card containing the DSP */
...
//    DspStatus, DSP_INIT_STATUS, pDspIfBlk[DspId], HOST_INIT_STATUS);
        ...
        ((DspStatus == HOST_INIT_STATUS) && (pDspIfBlk[DspId] == 0))) 
    {
        ...
        gpakReadDspMemory(rh, DspId, IfBlockPntr + NUM_CHANNELS_OFFSET, 1,
                          &DspChannels);
        ...
        gpakReadDspMemory(rh, DspId, IfBlockPntr + NUM_CONFERENCES_OFFSET, 1,
                          &DspConfs);
        ...
        gpakWriteDspMemory(rh, DspId, IfBlockPntr + DSP_STATUS_OFFSET, 1,
                           &DspStatus);
    ...
    )
{
    ...
    gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + CMD_MSG_LEN_OFFSET, 1,
                      &CmdMsgLength);
    if (CmdMsgLength != 0)
        return (0);
    ...
    )
{
    ...
    gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + REPLY_MSG_LEN_OFFSET, 1,
                      &MsgLength);
    if (MsgLength == 0)
        return (0);
...
//    DSP_WORD *pDebugData;   /* debug data buffer pointer in event data struct */
    ...
    gpakWriteDspMemory(rh, DspId, EventInfoAddress + CB_BUFR_TAKE_INDEX, 1,
                       &TakeIndex);
    ...
    )
{
    ...
    /* Attempt to send the message to the DSP and receive it's
       reply. */
    //need_reply_len;
	if (!TransactCmd(rh, DspId, MsgBuffer, 8, MSG_SERIAL_TXVAL_REPLY, 4,
	                                                    1, ChannelId))
    ...
    )
{
    ...
    gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + CPU_USAGE_OFFSET, 2,
                      ReadBuffer);
    ...
    )
{
    ...
    if (rh->dsp_type == DSP_5510)
        gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + FRAMING_STATS_OFFSET, 10, ReadBuffer);
    else
        gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + FRAMING_STATS_OFFSET, 4, ReadBuffer);
 ...
 *
 */
gpakDownloadStatus_t gpakDownloadDsp_5510(
    struct r1t1 *rh, /* Card containing the DSP */
