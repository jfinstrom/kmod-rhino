...
//            printk("r1t1: HRDY %s BUSY_N %s NEW %s Loop %d temp %x\n", (temp & R1T1_HRDY) ? "ON" : "OFF",
//            (temp & R1T1_BUSY_N) ? "ON" : "OFF", (temp & R1T1_D_NEW) ? "ON" : "OFF", loops, temp);

        }

//    printk(". %x %x", temp, flags);
//    printk("r1t1: loop timed out waiting for HPI %x %x\n", temp, flags);
    }
    return;
}
...
void r1t1_card_hpic_set(struct r1t1 *rh, __u16 hpic_data)
{
    ...
    __r1t1_card_pci_out(rh, TARG_REGS + R1T1_HPIC, ((hpi_lock & ~0x1F) | HPI_SEL | DSP_RST));
...
//            printk("r1t1: DSP %x set hpi_xadd %x u_nib %x\n", rh->dsp_sel, rh->hpi_xadd[rh->dsp_sel], u_nib);
...
__u16 r1t1_card_dsp_get(struct r1t1 *rh, __u32 dsp_address)
{
    ...
    if (rh->dsp_type == DSP_5510) {
        ...
        if (!(rh->hpi_xadd == u_nib)) {
//            printk("r1t1: DSP %x get hpi_xadd %x u_nib %x\n", rh->dsp_sel, rh->hpi_xadd[rh->dsp_sel], u_nib);
            rh->hpi_xadd = u_nib;
...
//    printk("r1t1: HPI reading %x from address %x\n", dsp_data, dsp_address);
...
//    spin_lock_irqsave(&rh->lock, gpak_flags);
...
//    spin_unlock_irqrestore(&rh->lock, gpak_flags);
    ...
    )
{
    ...
    if (!(stats)) {
        ...
        printk("r1t1: %d G168 DSP App file size = %d %x\n", rh->num+1, (unsigned int) &_binary_GpakDsp_fw_size, (unsigned int) &_binary_GpakDsp_fw_size);
    }
