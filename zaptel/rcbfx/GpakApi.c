...
//    DspStatus, DSP_INIT_STATUS, pDspIfBlk[DspId], HOST_INIT_STATUS);
        ...
        ((DspStatus == HOST_INIT_STATUS) && (pDspIfBlk[DspId] == 0)))
    {
        ...
        if (rcb_card->dsp_type == DSP_5510) {
            ...
            gpakReadDspMemory(rcb_card, DspId, IfBlockPntr + EVENT_MSG_PNTR_OFFSET, 2, Temp);
    ...
    )
{
    ...
    if (rcb_card->dsp_type == DSP_5510)
        gpakReadDspMemory(rcb_card, DspId, pDspIfBlk[DspId] + FRAMING_STATS_OFFSET, 10, ReadBuffer);
    else
        gpakReadDspMemory(rcb_card, DspId, pDspIfBlk[DspId] + FRAMING_STATS_OFFSET, 4, ReadBuffer);
 ...
 *
 */
gpakDownloadStatus_t gpakDownloadLoader(
    struct rcb_card_t  *rcb_card,     /* Card containing the DSP */
 ...
 *
 */
gpakDownloadStatus_t gpakDownloadDsp_5507(
    struct rcb_card_t  *rcb_card,     /* Card containing the DSP */
    unsigned short DspId,             /* DSP Identifier (0 to MaxDSPCores-1) */
