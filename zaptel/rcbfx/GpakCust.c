...
//            printk("HRDY %s BUSY_N %s NEW %s Loop %d temp %x\n", (temp & RCB_HRDY) ? "ON" : "OFF",
//            (temp & RCB_BUSY_N) ? "ON" : "OFF", (temp & RCB_D_NEW) ? "ON" : "OFF", loops, temp);

        }
        
//    printk("rcbfx: loop timed out waiting for HPI %x %x\n", temp, flags);
...
__u16 rcb_card_dsp_get(struct rcb_card_t *rcb_card, __u32 dsp_address)
{
    ...
    if (rcb_card->dsp_type == DSP_5510) {
        ...
        if (!(rcb_card->hpi_xadd == u_nib)) {
//            printk("rcbfx: hpi_xadd %x u_nib %x\n", rcb_card->hpi_xadd, u_nib);
...
void gpakReadDspMemory(
    struct rcb_card_t *rcb_card, /* Card containing the DSP */
...
//    spin_lock_irqsave(&rcb_card->lock, gpak_flags);
...
//    spin_unlock_irqrestore(&rcb_card->lock, gpak_flags);
...
//        printk("rcbfx writing DSP address %x with %x\n", DspAddress + word_num, pWordValues[word_num]);
    ...
    for (word_num=0;word_num<NumWords;word_num++) { // read NumWords from auto increment data register
        ...
        rec_data = rcb_card_dsp_get(rcb_card, DspAddress + word_num);

        if ((rec_data != pWordValues[word_num]) && ((DspAddress + word_num)!= 0x3801)) {
//            printk("rcbfx: check write DSP address %x with %x should be %x\n", DspAddress + word_num, rec_data, pWordValues[word_num]);
        }
    }
    ...
    )
{
    ...
    if (!(stats)) {
        ...
        printk("rcbfx %d: G168 DSP App file size = %d %x\n",rcb_card->pos+1, (unsigned int) &_binary_GpakDsp10_fw_size, (unsigned int) &_binary_GpakDsp10_fw_size);
    }
    ...
    )
{
    ...
    if (!(stats)) {
        ...
        if (rcb_card->num_chans > 4)
            printk("rcbfx %d: G168 07 08 DSP Loader file size = %d App file size = %d\n",rcb_card->pos+1, (unsigned int) &_binary_DspLoader_fw_size, (unsigned int) &_binary_GpakDsp0708_fw_size);
        else
            printk("rcbfx %d: G168 07 04 DSP Loader file size = %d App file size = %d\n",rcb_card->pos+1, (unsigned int) &_binary_DspLoader_fw_size, (unsigned int) &_binary_GpakDsp0704_fw_size);
    }
    ...
    if (FileId == loader_file) {
        for (byte_num=0;byte_num<NumBytes;byte_num++) {
            if ((file_size[loader_file] - 1) >= file_pos[loader_file][DspId]) {
                pBuffer[byte_num] =  _binary_DspLoader_fw_start[file_pos[loader_file][DspId]++];
            }
        }
    }
    ...
    if (FileId == app_file) {
        for (byte_num=0;byte_num<NumBytes;byte_num++) {
            if ((file_size[app_file] - 1) >= file_pos[app_file][DspId]) {
                if (rcb_card->num_chans > 4)
                    pBuffer[byte_num] = _binary_GpakDsp0708_fw_start[file_pos[app_file][DspId]++];
                else
                    pBuffer[byte_num] = _binary_GpakDsp0704_fw_start[file_pos[app_file][DspId]++];
            }
        }
    }
