...
//    DspStatus, DSP_INIT_STATUS, pDspIfBlk[DspId], HOST_INIT_STATUS);
        ...
        ((DspStatus == HOST_INIT_STATUS) && (pDspIfBlk[DspId] == 0))) 
    {
        ...
        if (rxt1_card->dsp_type == DSP_5510) {
            ...
            gpakReadDspMemory(rxt1_card, DspId, IfBlockPntr + EVENT_MSG_PNTR_OFFSET, 2, Temp);
    ...
    )
{
    ...
    gpakReadDspMemory(rxt1_card, DspId, pDspIfBlk[DspId] + REPLY_MSG_PNTR_OFFSET, 2, Temp);
    RECONSTRUCT_LONGWORD(BufferPointer, Temp);
    ...
    )
{
    ...
    if (rxt1_card->dsp_type == DSP_5510) {
        ...
        /* Attempt to send the Configure Serial Ports message to the DSP and receive
           it's reply. */
	    if (!TransactCmd(rxt1_card, DspId, MsgBuffer, 62, MSG_CONFIG_PORTS_REPLY, 4, 0, 0))
        ...
        /* Attempt to send the Configure Serial Ports message to the DSP and receive
           it's reply. */
	    if (!TransactCmd(rxt1_card, DspId, MsgBuffer, 26, MSG_CONFIG_PORTS_REPLY, 4, 0, 0))
            return (CpsDspCommFailure);
    }
...
#if 0
    printk("r%dt1 %d: Alg control Chan %d Control code %d\n",rxt1_card->numspans, rxt1_card->num+1 , ChannelId, ControlCode);
#endif
    ...
    )
{
    ...
    //need_reply_len;
	if (!TransactCmd(rxt1_card, DspId, MsgBuffer, 2, MSG_RESET_USAGE_STATS_REPLY, 4, 0, 0))
    ...
    )
{
    ...
    if (rxt1_card->dsp_type == DSP_5510)
        gpakReadDspMemory(rxt1_card, DspId, pDspIfBlk[DspId] + FRAMING_STATS_OFFSET, 10, ReadBuffer);
    else
        gpakReadDspMemory(rxt1_card, DspId, pDspIfBlk[DspId] + FRAMING_STATS_OFFSET, 4, ReadBuffer);
    ...
    )
{
    ...
    /* Attempt to send the message to the DSP and receive it's reply. */
    //need_reply_len;
	if (!TransactCmd(rxt1_card, DspId, MsgBuffer, 2, MSG_RESET_FRAME_STATS_REPLY, 4, 0, 0))
