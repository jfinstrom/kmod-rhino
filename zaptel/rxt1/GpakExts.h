...
extern struct chanInfo *initChanStruct(
	USHORT *saramBlock,		// pointer to channel's SARAM memory block
...
extern SPCStatus_t ConfigureGpakPcm(
	GpakSlotCfg_t SlotsSelect[],	// port's Slot selection
...
extern chanInfo_t **GetPcmToPktQueueHead(
	int SamplesPerFrame			// samples per frame (framing rate)
	);

// Get the address of Pkt to PCM framing rate queue head.
extern chanInfo_t **GetPktToPcmQueueHead(
	int SamplesPerFrame			// samples per frame (framing rate)
	);

// Determine the address of a framing rate phase counter.
extern USHORT *FrameRatePhaseCount(
	int SamplesPerFrame			// samples per frame (framing rate)
	);

// Adjust Far Echo and Bulk Delay buffer indices.
extern void AdjustFarBulkIndices(
	int WriteFrameSize,			// Bulk writer's frame size (samples per frame)
...
extern void EnqueueChannel(
	chanInfo_t *pChanInfo,		// pointer to channel's Channel Info
...
extern ACStatus_t ActivateGpakChannel(
	chanInfo_t *pChanInfo		// pointer to Channel Info
	);

// Deactivate a G.PAK Channel.
extern void DeactivateGpakChannel(
	chanInfo_t *pChanInfo		// pointer to Channel Info
	);

// Determine if the frame size is valid.
extern int ValidFrameSize(
	int FrameSize				// Frame Size
	);


// Copy linear buffer to circular buffer.
extern void copyLinearToCirc(
	USHORT *src,
	CircBufInfo_t *dest,
	USHORT len
	);
...
extern void ProcessVadToneEncode(
	chanInfo_t *pChanInfo,	// pointer to Channel Info
...
extern G168ChannelInstance_t * InitEchoCanceller(
	USHORT FrameSize,			// number of samples per frame
...
extern void algorithmControl(
	chanInfo_t      *pChan	    	// pointer to Channel structure
    );


extern void writeTransmitEnables(
	USHORT McBspId,				// McBSP Id
