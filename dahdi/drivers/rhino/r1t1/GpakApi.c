 ...
 *
 */
static int CheckDspReset(struct r1t1 *rh,	/* Card containing the DSP */
...
//    unsigned short int i;    /* loop index / counter */
	...
	gpakReadDspMemory(rh, DspId, DSP_IFBLK_ADDRESS, 2, Temp);
...
//    printk("IfBlockPntr %x\n", IfBlockPntr);
	...
	gpakReadDspMemory(rh, DspId, IfBlockPntr + DSP_STATUS_OFFSET, 1, &DspStatus);
...
//    DspStatus, DSP_INIT_STATUS, pDspIfBlk[DspId], HOST_INIT_STATUS);
	if (DspStatus == DSP_INIT_STATUS ||
		...
		gpakReadDspMemory(rh, DspId, IfBlockPntr + MAX_CMD_MSG_LEN_OFFSET, 1,
						  &(MaxCmdMsgLen[DspId]));

		/* read the number of configured DSP channels */
		gpakReadDspMemory(rh, DspId, IfBlockPntr + NUM_CHANNELS_OFFSET, 1, &DspChannels);
		...
		gpakReadDspMemory(rh, DspId, IfBlockPntr + NUM_CONFERENCES_OFFSET, 1, &DspConfs);
		...
		gpakReadDspMemory(rh, DspId, IfBlockPntr + NUM_PKT_CHANNELS_OFFSET, 1,
						  &DspChannels);
		...
		gpakReadDspMemory(rh, DspId, IfBlockPntr + PKT_BUFR_MEM_OFFSET, 2, Temp);
		...
		if (rh->dsp_type == DSP_5510) {
			...
			gpakReadDspMemory(rh, DspId, IfBlockPntr + EVENT_MSG_PNTR_OFFSET, 2, Temp);
		...
		gpakWriteDspMemory(rh, DspId, IfBlockPntr + DSP_STATUS_OFFSET, 1, &DspStatus);
 ...
 *
 */
static int WriteDspCmdMessage(struct r1t1 *rh,	/* Card containing the DSP */
							  int DspId,	/* DSP Identifier (0 to MaxDSPCores-1) */
	...
	)
{
	...
	gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + CMD_MSG_LEN_OFFSET, 1, &CmdMsgLength);
	if (CmdMsgLength != 0)
		return (0);

	/* Purge any previous Reply message that wasn't read. */
	gpakWriteDspMemory(rh, DspId, pDspIfBlk[DspId] + REPLY_MSG_LEN_OFFSET, 1,
					   &CmdMsgLength);

	/* Copy the Command message into DSP memory. */
	gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + CMD_MSG_PNTR_OFFSET, 2, Temp);
	RECONSTRUCT_LONGWORD(BufferPointer, Temp);
	gpakWriteDspMemory(rh, DspId, BufferPointer, (MsgLength + 1) / 2, pMessage);

	/* Store the message length in DSP's Command message length (flags DSP that
	   a Command message is ready). */
	CmdMsgLength = MsgLength;
	gpakWriteDspMemory(rh, DspId, pDspIfBlk[DspId] + CMD_MSG_LEN_OFFSET, 1,
					   &CmdMsgLength);
 ...
 *
 */
static int ReadDspReplyMessage(struct r1t1 *rh,	/* Card containing the DSP */
							   int DspId,	/* DSP Identifier (0 to MaxDSPCores-1) */
	...
	)
{
	...
	gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + REPLY_MSG_LEN_OFFSET, 1, &MsgLength);
	if (MsgLength == 0)
		return (0);
	...
	gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + REPLY_MSG_PNTR_OFFSET, 2, Temp);
	RECONSTRUCT_LONGWORD(BufferPointer, Temp);
	gpakReadDspMemory(rh, DspId, BufferPointer, (MsgLength + 1) / 2, pMessage);
	...
	*pMsgLength = MsgLength;
	...
	MsgLength = 0;
	gpakWriteDspMemory(rh, DspId, pDspIfBlk[DspId] + REPLY_MSG_LEN_OFFSET, 1, &MsgLength);
 ...
 *
 */
static void ReadCircBuffer(struct r1t1 *rh,	/* Card containing the DSP */
						   int DspId,	/* DSP Identifier (0 to MaxDSPCores-1) */
	...
	)
{
	...
	if (NumWords > WordsTillEnd) {
		gpakReadDspMemory(rh, DspId, *TakeAddress, WordsTillEnd, pWordBuffer);
		gpakReadDspMemory(rh, DspId, BufrBaseAddress, NumWords - WordsTillEnd,
	...
	/* If a buffer wrap will not occur, read all words starting at the current
	   take address in the buffer. */
		...
		gpakReadDspMemory(rh, DspId, *TakeAddress, NumWords, pWordBuffer);
		if (NumWords == WordsTillEnd)
 ...
 *
 */
static unsigned int TransactCmd(struct r1t1 *rh,	/* Card containing the DSP */
								int DspId,	/* DSP Identifier (0 to MaxDSPCores-1) */
	...
	)
{
	...
	gpakLockAccess(rh, DspId);


	/* Attempt to write the command message to the DSP. */
	LoopCount = 0;
	while ((FuncStatus = WriteDspCmdMessage(rh, DspId, pMsgBufr, CmdLength)) != 1) {
		if (FuncStatus == -1)
			break;
	...
	if (FuncStatus == 1) {
		for (LoopCount = 0; LoopCount < MAX_WAIT_LOOPS; LoopCount++) {
			...
			FuncStatus = ReadDspReplyMessage(rh, DspId, pMsgBufr, &RcvReplyLength);
			if (FuncStatus == 1) {
 ...
 *
 */
gpakConfigPortStatus_t gpakConfigurePorts(struct r1t1 * rh,	/* Card containing the DSP */
	...
	)
{
	...
	if (rh->dsp_type == DSP_5510) {
		...
		/* Attempt to send the Configure Serial Ports message to the DSP and receive
		   it's reply. */
		if (!TransactCmd(rh, DspId, MsgBuffer, 62, MSG_CONFIG_PORTS_REPLY, 4, 0, 0))
		...
		/* Attempt to send the Configure Serial Ports message to the DSP and receive
		   it's reply. */
		if (!TransactCmd(rh, DspId, MsgBuffer, 26, MSG_CONFIG_PORTS_REPLY, 4, 0, 0))
			return (CpsDspCommFailure);
	}
 ...
 *
 */
gpakConfigChanStatus_t gpakConfigureChannel(struct r1t1 * rh,	/* Card containing the DSP */
											unsigned short int DspId,	/* DSP Id (0 to MaxDSPCores-1) */
	...
	)
{
	...
	/* Attempt to send the Configure Channel message to the DSP and receive it's
	   reply. */
	if (!TransactCmd(rh, DspId, MsgBuffer, MsgLength, MSG_CONFIG_CHAN_REPLY, 4, 1,
					 (DSP_WORD) ChannelId))
 ...
 *
 */
gpakTearDownStatus_t gpakTearDownChannel(struct r1t1 * rh,	/* Card containing the DSP */
										 unsigned short int DspId,	/* DSP Id (0 to MaxDSPCores-1) */
										 unsigned short int ChannelId,	/* Channel Id (0 to MaxChannels-1) */
	...
	)
{
	...
	/* Attempt to send the Tear Down Channel message to the DSP and receive it's
	   reply. */
	if (!TransactCmd(rh, DspId, MsgBuffer, 3, MSG_TEAR_DOWN_REPLY, 4, 1,
					 (DSP_WORD) ChannelId))
 ...
 *
 */
gpakAlgControlStat_t gpakAlgControl(struct r1t1 * rh,	/* Card containing the DSP */
...
//    printk("R1T1 %d: Alg control Chan %d Control code %d\n", rh->num+1 , ChannelId, ControlCode);

	/* Attempt to send the Tear Down Channel message to the DSP and receive it's
	   reply. */
	//need_reply_len;       
	if (!TransactCmd(rh, DspId, MsgBuffer, 4, MSG_ALG_CONTROL_REPLY, 4, 1,
					 (DSP_WORD) ChannelId))
 ...
 */
gpakReadEventFIFOMessageStat_t gpakReadEventFIFOMessage(struct r1t1 * rh,	// Card containing the DSP
														unsigned short int DspId,	// DSP identifier
...
//    DSP_WORD *pDebugData;   /* debug data buffer pointer in event data struct */
	...
	gpakReadDspMemory(rh, DspId, EventInfoAddress, CIRC_BUFFER_INFO_STRUCT_SIZE,
					  WordBuffer);
	...
	ReadCircBuffer(rh, DspId, BufrBaseAddress, BufrLastAddress, &TakeAddress,
				   WordBuffer, 2);
	TakeIndex += 2;
	...
	gpakWriteDspMemory(rh, DspId, EventInfoAddress + CB_BUFR_TAKE_INDEX, 1, &TakeIndex);
 ...
 *  Status  code indicating success or a specific error.
 */
gpakPingDspStat_t gpakPingDsp(struct r1t1 * rh,	/* Card containing the DSP */
							  unsigned short int DspId,	// DSP identifier
	...
	)
{
	...
	/* Attempt to send the ping message to the DSP and receive it's
	   reply. */
	if (!TransactCmd(rh, DspId, MsgBuffer, 1, MSG_PING_REPLY, 6, 0, 0))
 ...
 *  Status  code indicating success or a specific error.
 */
gpakSerialTxFixedValueStat_t gpakSerialTxFixedValue(struct r1t1 * rh,	/* Card containing the DSP */
													unsigned short int DspId,	// DSP identifier
													unsigned short int ChannelId,	// channel identifier
	...
	)
{
	...
	/* Attempt to send the message to the DSP and receive it's
	   reply. */
	//need_reply_len;
	if (!TransactCmd(rh, DspId, MsgBuffer, 8, MSG_SERIAL_TXVAL_REPLY, 4, 1, ChannelId))
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
gpakControlTdmLoopBackStat_t gpakControlTdmLoopBack(struct r1t1 * rh,	/* Card containing the DSP */
													unsigned short int DspId,	// DSP identifier
	...
	)
{
	...
	/* Attempt to send the message to the DSP and receive it's
	   reply. */
	//need_reply_len;
	if (!TransactCmd(rh, DspId, MsgBuffer, 4, MSG_TDM_LOOPBACK_REPLY, 4, 0, 0))
 ...
 *
 */
gpakReadCpuUsageStat_t gpakReadCpuUsage(struct r1t1 * rh,	/* Card containing the DSP */
										unsigned short int DspId,	// Dsp Identifier
	...
	)
{
	...
	gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + CPU_USAGE_OFFSET, 2, ReadBuffer);
 ...
 *  Status  code indicating success or a specific error.
 */
gpakResetCpuUsageStat_t gpakResetCpuUsageStats(struct r1t1 * rh,	/* Card containing the DSP */
											   unsigned short int DspId	// DSP identifier
	)
{
	DSP_WORD MsgBuffer[MSG_BUFFER_SIZE];	/* message buffer */
	...
	//need_reply_len;
	if (!TransactCmd(rh, DspId, MsgBuffer, 2, MSG_RESET_USAGE_STATS_REPLY, 4, 0, 0))
 ...
 *  Status  code indicating success or a specific error.
 */
gpakReadFramingStatsStatus_t gpakReadFramingStats(struct r1t1 * rh,	/* Card containing the DSP */
												  unsigned short int DspId,	// DSP identifier
	...
	)
{
	...
	if (rh->dsp_type == DSP_5510)
		gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + FRAMING_STATS_OFFSET, 10,
						  ReadBuffer);
	else
		gpakReadDspMemory(rh, DspId, pDspIfBlk[DspId] + FRAMING_STATS_OFFSET, 4,
						  ReadBuffer);
	...
	*pDmaStopErrorCount = ReadBuffer[3];
	...
	if ((pDmaSlipStatsBuffer != 0) && (rh->dsp_type == DSP_5510))
 ...
 *  Status  code indicating success or a specific error.
 */
gpakResetFramingStatsStatus_t gpakResetFramingStats(struct r1t1 * rh,	/* Card containing the DSP */
													unsigned short int DspId	// DSP identifier
	)
{
	DSP_WORD MsgBuffer[MSG_BUFFER_SIZE];	/* message buffer */
	...
	/* Attempt to send the message to the DSP and receive it's reply. */
	//need_reply_len;
	if (!TransactCmd(rh, DspId, MsgBuffer, 2, MSG_RESET_FRAME_STATS_REPLY, 4, 0, 0))
 ...
 *
 */
gpakDownloadStatus_t gpakDownloadDsp_5510(struct r1t1 * rh,	/* Card containing the DSP */
	...
	)
{
	...
	while (RetStatus == GdlSuccess) {
		...
		NumRead = gpakReadFile_5510(rh, FileId, DlByteBufr, 6);
		if (NumRead == -1) {
		...
		while (WordCount != 0) {
			...
			WordCount -= NumWords;
			NumRead = gpakReadFile_5510(rh, FileId, DlByteBufr, NumWords * 2);
			if (NumRead == -1) {
				RetStatus = GdlFileReadError;
				break;
			}
			...
			while (check_count < 4) {
				gpakWriteDspMemory(rh, DspId, Address, NumWords, DlWordBufr);
				gpakReadDspMemory(rh, DspId, Address, NumWords, DlWordChek);
 ...
 *
 */
gpakReadDSPMemoryStat_t gpakReadDSPMemoryMap(struct r1t1 * rh,	// Card containing the DSP
											 unsigned short int DspId,	// Dsp Identifier
	...
	)
{
	...
	/* Attempt to send the Read memory section message to the DSP and receive it's
	   reply. */
	//need_reply_len;       
	if (!TransactCmd(rh, DspId, MsgBuffer, 8, MSG_READ_DSP_MEMORY_REPLY,
